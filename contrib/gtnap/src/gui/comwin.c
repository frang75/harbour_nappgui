...
const char_t *comwin_open_file(Window *parent, const char_t **ftypes, const uint32_t size, const char_t *caption, const char_t *start_dir)
{
    const GuiCtx *context = guictx_get_current();
    void *ositem = parent ? _window_ositem(parent) : NULL;
    cassert_no_null(context);
    return context->func_comwin_file(ositem, ftypes, size, caption, start_dir, TRUE);
}

/*---------------------------------------------------------------------------*/

const char_t *comwin_save_file(Window *parent, const char_t **ftypes, const uint32_t size, const char_t *caption, const char_t *start_dir)
{
    const GuiCtx *context = guictx_get_current();
    void *ositem = parent ? _window_ositem(parent) : NULL;
    cassert_no_null(context);
    return context->func_comwin_file(ositem, ftypes, size, caption, start_dir, FALSE);
}
