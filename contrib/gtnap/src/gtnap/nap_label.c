...
HB_FUNC(NAP_LABEL)
{
    ...
    uint32_t id = gtnap_label(wid, top, left, text_block, in_scroll);
    hb_retni(id);
}
...
HB_FUNC(NAP_LABEL_MESSAGE)
{
    ...
    uint32_t id = gtnap_label_message(wid, top, left, in_scroll);
    hb_retni(id);
}
...
HB_FUNC(NAP_LABEL_UPDATE)
{
    ...
    gtnap_label_update(wid, id, top, left, text_block);
}
...
HB_FUNC(NAP_LABEL_FGCOLOR)
{
    ...
    gtnap_label_fgcolor(wid, id, color);
}
...
HB_FUNC(NAP_LABEL_BGCOLOR)
{
    ...
    uint32_t id = hb_parni(2);
    color_t color = (color_t)hb_parni(3);
    gtnap_label_bgcolor(wid, id, color);
}
...
HB_FUNC(NAP_LABEL_COLOR)
{
    ...
    gtnap_label_color(wid, id, hb_color);
}
