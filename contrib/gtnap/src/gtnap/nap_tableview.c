...
HB_FUNC(NAP_TABLEVIEW)
{
    ...
    uint32_t id = gtnap_tableview(wid, top, left, bottom, right, multisel, autoclose, in_scroll);
    hb_retni(id);
}
...
HB_FUNC(NAP_TABLEVIEW_COLUMN)
{
    ...
    gtnap_tableview_column(wid, id, width, head_block, eval_block);
}
...
HB_FUNC(NAP_TABLEVIEW_SCROLL)
{
    ...
    gtnap_tableview_scroll(wid, id, horizontal, vertical);
}
...
HB_FUNC(NAP_TABLEVIEW_GRID)
{
    ...
    gtnap_tableview_grid(wid, id, hlines, vlines);
}
...
HB_FUNC(NAP_TABLEVIEW_HEADER)
{
    ...
    gtnap_tableview_header(wid, id, visible);
}
...
HB_FUNC(NAP_TABLEVIEW_FREEZE)
{
    ...
    gtnap_tableview_freeze(wid, id, col_id);
}
...
HB_FUNC(NAP_TABLEVIEW_BIND_AREA)
{
    ...
    gtnap_tableview_bind_area(wid, id, while_block);
}
...
HB_FUNC(NAP_TABLEVIEW_BIND_DATA)
{
    ...
    gtnap_tableview_bind_data(wid, id, num_rows);
}
...
HB_FUNC(NAP_TABLEVIEW_DESELECT_ALL)
{
    uint32_t wid = hb_parni(1);
    uint32_t id = hb_parni(2);
    gtnap_tableview_deselect_all(wid, id);
}
...
HB_FUNC(NAP_TABLEVIEW_SELECT_ROW)
{
    ...
    cassert(row_id > 0);
    gtnap_tableview_select_row(wid, id, row_id - 1);
}
...
HB_FUNC(NAP_TABLEVIEW_TOGGLE_ROW)
{
    ...
    uint32_t row_id = hb_parni(3);
    cassert(row_id > 0);
    gtnap_tableview_toggle_row(wid, id, row_id - 1);
}
...
HB_FUNC(NAP_TABLEVIEW_SELECTED_ROWS)
{
    uint32_t wid = hb_parni(1);
    uint32_t id = hb_parni(2);
    const ArrSt(uint32_t) *selected = gtnap_tableview_selected_rows(wid, id);
...
HB_FUNC(NAP_TABLEVIEW_FOCUS_ROW)
{
    uint32_t wid = hb_parni(1);
    uint32_t id = hb_parni(2);
    uint32_t row = gtnap_tableview_focus_row(wid, id) + 1;
    hb_retni(row);
}
...
HB_FUNC(NAP_TABLEVIEW_RECNO_FROM_ROW)
{
    ...
    uint32_t recno;
    cassert(row_id > 0);
    recno = gtnap_tableview_recno_from_row(wid, id, row_id - 1);
    hb_retni(recno);
}
...
HB_FUNC(NAP_TABLEVIEW_ROW_FROM_RECNO)
{
    ...
    uint32_t row;
    row = gtnap_tableview_row_from_recno(wid, id, recno);
    hb_retni(row + 1);
}
...
HB_FUNC(NAP_TABLEVIEW_REFRESH_CURRENT)
{
    uint32_t wid = hb_parni(1);
    uint32_t id = hb_parni(2);
    gtnap_tableview_refresh_current(wid, id);
}
...
HB_FUNC(NAP_TABLEVIEW_REFRESH_ALL)
{
    uint32_t wid = hb_parni(1);
    uint32_t id = hb_parni(2);
    gtnap_tableview_refresh_all(wid, id);
}
