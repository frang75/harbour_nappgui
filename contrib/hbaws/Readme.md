...
> **Important:** The AWSSDK binaries corresponding to each build (`awssdk/bin/win/msvc`), (`awssdk/bin/linux/gcc`), etc. must be distributed with the final application for it to work.
...
On **Linux/macOS** `tests/harbour/test.sh` will perform the compilation, setting `LD_LIBRARY_PATH` or `DYLD_LIBRARY_PATH`.
